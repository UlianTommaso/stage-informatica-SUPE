...
#let localization_it = (
  ...
  "Grader": "Grader",
  "InputFile": "Dati di input",
  "OutputFile": "Dati di output",
)
...
#let inputfile = context [ == #localize(text.lang, "InputFile") <inputfile> ]
#let outputfile = context [ == #localize(text.lang, "OutputFile") <outputfile> ]
...
#let statement(doc, title: none) = {
  ...
  show heading: set text(font: ("CMU Sans Serif"))
  ...
  let syllabus_text = context {
    if syllabus != none {
      for _ in range(0, syllabus) {
        text("★", fill: orange, size: 1.6em)
      }
      for _ in range(syllabus, 5) {
        text("★", fill: gray, size: 1.6em)
      }
    }
  }
