...
Via Gemona, la via dove si trova il Toppo, è composta da $N$ luoghi, numerati da $1$ a $N$, disposti in fila lungo la strada. I luoghi sono collegati in sequenza: il luogo $1$ è collegato al luogo $2$, il luogo $2$ è collegato al luogo $3$, e così via fino al luogo $N$.

Ci sono $N$ partecipanti allo stage, uno per ogni luogo da $1$ a $N$. Il Toppo (la sede dello stage dove tutti devono tornare) si trova al luogo $T$ ($1 \le T \le N$). Ogni partecipante deve tornare al Toppo partendo dal proprio luogo.
...
\InputFile
La prima riga contiene tre numeri interi $N$, $K$ e $T$ separati da spazi.

Ognuna delle successive $N-1$ righe contiene un intero $t_i$ che rappresenta i minuti necessari per andare dal luogo $i$ al luogo $i+1$.

Le successive $K$ righe descrivono ciascuna un chiosco di pizza con due interi: l'indice del suo luogo e il suo livello di soddisfazione. Più chioschi possono trovarsi nello stesso luogo.

\OutputFile
L'output deve consistere di $N$ righe. La riga $i$ contiene il singolo intero $1$ se il partecipante nel luogo $i$ può fermarsi a mangiare una pizza lungo il percorso verso il Toppo, e $0$ altrimenti. (Anche il partecipante che si trova già al Toppo può decidere di muoversi per prendere una pizza e poi tornare, se il tempo extra lo permette).
...
Partecipante 5:
\begin{itemize}
    \item Percorso diretto: già lì! (tempo: $0$ minuti).
    \item Con pizza al luogo $4$: $5 \to 4$ (prende la pizza) $\to 5$ (tempo: $3+3=6$ minuti).
    \item Tempo extra: $6$, soddisfazione: $6 \implies$ può fermarsi! ($6 \le 6$) \checkmark
\end{itemize}
